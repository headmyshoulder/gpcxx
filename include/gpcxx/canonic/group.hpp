...
public:
    ...
    {
        if( m_operation.count_nodes() > 0 ) throw gpcxx_exception( "Group operation node contains children. It cannot be used as a prototyp." );
        if( m_identity.count_nodes() > 0 ) throw gpcxx_exception( "Group identity operation node contains children. It cannot be used as a prototyp." );
        if( m_inverse_operation.count_nodes() > 0 ) throw gpcxx_exception( "Group inverse operation node contains children. It cannot be used as a prototyp." );
        if( m_inverse_function.count_nodes() > 0 ) throw gpcxx_exception( "Group inverse function node contains children. It cannot be used as a prototyp." );
    }
