...
TEST( TESTNAME , transform_unary )
{
    ...
    for( auto const nthreads : thread_count )
    {
        for( auto i = rng_size_min; i < rng_size_max; i+= rng_size_step)
        {
            ...
            std::vector<int> out_singel_thread( boost::size( rng1 ), 0 );
            std::vector<int> out_n_thread( boost::size( rng1 ), 1 );
            
            boost::transform( rng1, rng2, boost::begin( out_singel_thread ), fun );
            gpcxx::par::transform( rng1, rng2, boost::begin( out_n_thread ), fun, 2 );
            EXPECT_EQ( out_singel_thread , out_n_thread );
        }
    }
}
